<template>
	<view>
		<view v-if="list.length" :class="['goods-list', goodsValue.template, goodsValue.style]"
			:style="goodsListWarpCss">
		...
		<ns-goods-sku-index ref="goodsSkuIndex" @cartListChange="cartListChange"
			@addCart="addCartPoint"></ns-goods-sku-index>
		...
		methods: {
				...
				this.$api.sendRequest({
					...
					success: res => {
						if (res.code == 0 && res.data) {
							...
							// 切屏滚动，每页显示固定数量
							if (this.goodsValue.template == 'horizontal-slide' && this.goodsValue
								.slideMode == 'slide') {
								let size = 3;
								let temp = [];
			...
			cartListChange(e) {
				...
				this.list.forEach((item, index) => {
					let cartItem = cart['goods_' + item.goods_id] && cart['goods_' + item.goods_id]['sku_' +
						item.sku_id
					] ? cart['goods_' + item.goods_id]['sku_' + item.sku_id] : null;
