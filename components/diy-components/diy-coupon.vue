<template>
	<view class="coupon-wrap" :class="'coupon-box-' + value.style" v-if="computedCouponList.length > 0" :style="[
				...
				<swiper-item v-for="(numItem, numIndex) in Math.ceil(computedCouponList.length / 3)" :key="numIndex"
					...
					<view class="coupon-item" v-for="(item, index) in computedCouponList" :key="index"
						...
						<view class="coupon-type font-size-tag">
							<text
								:style="{ color: value.limitColor }">{{ item.at_least > 0 ? '满' + Number(item.at_least) + '元可用' : '无门槛优惠券' }}</text>
					...
					<view class="coupon" v-for="(item, index) in computedCouponList" :key="index"
						...
						<view class="coupon-content">
							<view class="price-wrap">
								<text class="price"
									:style="{ color: value.moneyColor }">{{ (item.discount == '0.00' ? item.money : item.discount) | moneyConduct }}</text>
				...
				<view class="coupon-list" v-for="(item, index) in computedCouponList" :key="index"
					...
					<view class="coupon">
						...
						<view class="coupon-type">
							...
							<view class="coupon-least" :style="{ color: value.limitColor }">
								有效期至{{ $util.timeStampTurnTime(item.end_time, 'yearmonthday') }}</view>
						</view>
						<view class="coupon-line"></view>
						<view class="coupon-get" :style="couponBtnStyle" v-if="item.useState == 0">
							{{ value.btnStyle.text || '立即领取' }}
