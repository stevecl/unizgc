				...
				<view class="template-left">
					<view :class="['item', value.mode]" @click="$util.diyRedirectTo(value.list[0].link)"
						...
						<image :src="$util.img(value.list[0].imageUrl)" :mode="list[0].imageMode || 'scaleToFill'"
							:style="list[0].pageItemStyle" :show-menu-by-longpress="true"></image>
					</view>
				</view>
						...
						<template v-if="index > 0">
							<view :key="index" :class="['item', value.mode]" @click="$util.diyRedirectTo(item.link)"
								...
								<image :src="$util.img(item.imageUrl)" :mode="item.imageMode || 'scaleToFill'"
									:style="item.pageItemStyle" :show-menu-by-longpress="true"></image>
							</view>
						</template>
					</template>
				</view>
				...
				<view class="template-right">
					<view :class="['item', value.mode]"
						...
						<image :src="$util.img(value.list[1].imageUrl)" :mode="list[1].imageMode || 'scaleToFill'"
							:style="list[1].pageItemStyle" :show-menu-by-longpress="true"></image>
					</view>
                	...
                	success: res => {
                        this.list.forEach((item, index) => {
                            if (index == 0) {
                                ...
                                item.imgWidth = res.windowWidth - uni.upx2px(this.value.margin
                                    .both * 4) - uni.upx2px(this.value.imageGap * 2);
                                item.imgWidth = item.imgWidth / 2;
                                item.imgHeight = item.imgWidth * ratio;
                                rightHeight = (item.imgHeight - uni.upx2px(this.value.imageGap *
                                    2)) / 2;
                                item.imgWidth += 'px';
                            } else {
                	...
                	success: res => {
                        this.list.forEach((item, index) => {
                            ...
                            if (index == 0) {
                                ...
                                item.imgWidth = this.list[0].imgWidth;
                                item.imgHeight = (this.list[0].imgHeight - uni.upx2px(this.value
                                    .imageGap * 2)) / 2;
                            } else if (index > 1) {
                                item.imgWidth = (this.list[0].imgWidth - uni.upx2px(this.value
                                    .imageGap * 2)) / 2;
