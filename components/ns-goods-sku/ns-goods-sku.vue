            ...
            <view class="sku-info" :style="{ height: skuHeight }">
                <view class="header">
                    <block v-if="type == 'point' && goodsDetail.type && goodsDetail.type != 1">
                        <view class="img-wrap" v-if="goodsDetail.type == 2">
                            <image
                                :src="goodsDetail.image ? $util.img(goodsDetail.image) : $util.img('public/uniapp/point/coupon.png')"
                                mode="aspectFit" />
                        </view>
                        <view class="img-wrap" v-if="goodsDetail.type == 3">
                            <image
                                :src="goodsDetail.image ? $util.img(goodsDetail.image) : $util.img('public/uniapp/point/hongbao.png')"
                                mode="aspectFit" />
                        </view>
                    </block>
                    ...
                    <view class="main">
                            ...
                            <block v-if="goodsDetail.type == 1">
                                <view class="price-wrap">
                                    ...
                                    <block v-if="goodsDetail.exchange_price != '0.00' && goodsDetail.pay_type > 0">
                                        ...
                                        <text
                                            class="price price-color font-size-toolbar price-font">{{ $lang('common.currencySymbol') }}{{ goodsDetail.exchange_price }}</text>
                                    </block>
                                </view>
                        ...
                        <block v-else>
                            ...
                            <view class="price-wrap" v-else-if="type == 'topic'">
                                ...
                                <text class="price price-style large">
                                    {{
										parseFloat(goodsDetail.topic_price)
											.toFixed(2)
											.split('.')[0]
									}}
                                </text>
                            ...
                            <view class="price-wrap" v-else>
                                ...
                                <block v-if="goodsDetail.discount_price > 0">
                                    <block v-if="goodsDetail.member_price">
                                        <text class="price price-style large"
                                            v-if="showPrice(goodsDetail.discount_price) < showPrice(goodsDetail.member_price)">
                                            {{
                                    ...
                                    <block v-else>
                                        <text class="price price-style large"
                                            v-if="showPrice(goodsDetail.discount_price) < showPrice(goodsDetail.price)">
                                            {{
												parseFloat(goodsDetail.discount_price)
													.toFixed(2)
													.split('.')[0]
                        ...
                        <view class="sku-name font-size-tag" v-if="goodsDetail.sku_spec_format">
                            ...
                            <text v-for="(item, index) in goodsDetail.sku_spec_format" :key="index"
                ...
                <view class="body-item">
                    <scroll-view scroll-y class="wrap">
                        ...
                        <view class="sku-list-wrap"
                            v-if="type == 'pintuan' && goodsDetail.pintuan_type == 'ladder' && !goodsDetail.group_id">
                            <text class="title font-size-base">&nbsp;</text>
                            <view class="items" :class="{ selected: pintuan_num_field == 'pintuan_num' }"
                                @click="pintuanChange('pintuan_num')">
                            ...
                            <view class="number-line">
                                ...
                                <view class="number">
                                    <button type="default" class="decrease color-line-border"
                                        :class="{ disabled: decreaseDisabled }" @click="changeNum('-')">-</button>
                                    <input
                                        type="number"
                                    ...
                                    <button type="default" class="increase color-line-border"
                                        :class="{ disabled: increaseDisabled }" @click="changeNum('+')">+</button>
                                    <!-- v-if="max_buy == 1" -->
                ...
                <view class="footer" @click="confirm()">
                    ...
                    <block v-else-if="goodsDetail.goods_state == 1">
                        <button type="primary"
                            v-if="goodsDetail.stock && goodsDetail.stock != 0 && type == 'join_cart'">加入购物车</button>
                        <button type="primary"
                            v-else-if="goodsDetail.stock && goodsDetail.stock != 0 && type == 'buy_now'">立即购买</button>
                        <button type="primary"
                            v-else-if="goodsDetail.stock && goodsDetail.stock != 0 && type == 'confirm'">确认</button>
...
<script>
    ...
    export default {
        ...
        computed: {
             ...
             */
            balance: function() {
                let price = this.goodsDetail.member_price && this.goodsDetail.member_price > 0 ? this.goodsDetail
                    .member_price : this.goodsDetail.price;
            ...
            increaseDisabled: function() {
                let max = this.maxBuy > 0 && this.maxBuy < this.goodsDetail.stock ? this.maxBuy : this.goodsDetail
                    .stock;
        ...
        methods: {
                        ...
                        if (res.code >= 0) {
                            ...
                            res.data.forEach((item, index) => {
                                ...
                                if (item.promotion_type == 1) {
                                    item.discountTimeMachine = this.$util.countDown(item.end_time -
                                        res.timestamp);
                                }

                                if (item.promotion_type == 1 && item.discountTimeMachine) {
                                    if (item.member_price > 0 && Number(item.member_price) <=
                ...
                this.$api.sendRequest({
                    ...
                    success: res => {
                        if (res.code >= 0) {
                            ...
                            res.data.forEach((item, index) => {
                                item = this.handleData(item);
                                item.show_price = this.goodsDetail.group_id == 0 ? item
                                    .promotion_price : item.pintuan_price;
                                item.save_price = item.price - item.show_price > 0 ? (item.price -
                                    item.show_price).toFixed(2) : 0;
                ...
                this.$api.sendRequest({
                    ...
                    success: res => {
                        if (res.code >= 0) {
                            ...
                            res.data.forEach((item, index) => {
                                item = this.handleData(item);
                                item.show_price = this.goodsDetail.bargain_id > 0 ? item
                    ...
                    success: res => {
                        if (res.code >= 0) {
                            ...
                            res.data.forEach((item, index) => {
                                ...
                                item.price - item.show_price > 0 ? (item.price - item.show_price)
                                    .toFixed(2) : 0;
                                obj['sku_' + item.sku_id] = item;
                            });
                            this.skuId = res.data[0].sku_id;
                ...
                this.$api.sendRequest({
                    ...
                    success: res => {
                        if (res.code >= 0) {
                            ...
                            res.data.forEach((item, index) => {
                                item = this.handleData(item);
                                item.show_price = this.goodsDetail.topic_id > 0 ? item.topic_price :
                                    item.price;
                                item.save_price = item.price - item.show_price > 0 ? (item.price -
                                    item.show_price).toFixed(2) : 0;
                                obj['sku_' + item.sku_id] = item;
                    ...
                    if (this.number < stock) {
                        ...
                        if (this.goodsDetail.is_limit == 1 && this.maxBuy > 0) {
                            ...
                            if (this.goodsDetail.limit_type == 2) {
                                ...
                                message += this.goodsDetail.purchased_num > 0 ? '，您已购买了' + this.goodsDetail
                                    .purchased_num + this.goodsDetail.unit : '';
                                this.$util.showToast({
                                    title: message
                                });
                                return;
            ...
            keyInput(flag, callback) {
                setTimeout(() => {
                    ...
                    if (this.type == 'pintuan' && this.goodsDetail.pintuan_id && this.number > this.goodsDetail
                        ...
                        //最低购买数量
                        this.number = 1;
                    } else if (this.type == 'seckill' && this.goodsDetail.seckill_id && this.goodsDetail
                        .max_buy > 0 && this.number > this.goodsDetail.max_buy) {
                        //限购数量大于库存总数取库存
                        this.number = this.goodsDetail.max_buy;
                    } else if (this.type == 'bargain' && this.goodsDetail.bargain_id) {
                        // 砍价
                        this.number = 1;
                    } else if (this.type == 'presale' && this.goodsDetail.presale_id && this.goodsDetail
                        .presale_num > 0 && this.number > this.goodsDetail.presale_num) {
                        // 预售
            ...
            confirm(isJoinCartBtn = false) {
                ...
                this.keyInput(true, () => {
                    ...
                    if ((this.type == 'join_cart' || isJoinCartBtn) && this.goodsDetail.is_limit == 1 && this
                        .maxBuy > 0 && this.cartNumber + this.number > this.maxBuy) {
                        this.$util.showToast({
