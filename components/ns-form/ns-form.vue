        ...
        <block v-for="(item, index) in formData">
            <view class="order-wrap" v-if="item.controller == 'Text'">
                <view class="order-cell">
                    ...
                    <view class="box"><input type="text" :placeholder="item.value.placeholder"
                            placeholder-class="placeholder color-tip" v-model="item.val" /></view>
                </view>
            </view>
            ...
            <view class="order-wrap" v-if="item.controller == 'Textarea'">
                <view class="order-cell flex-box textarea">
                    ...
                    <view class="box"><textarea :placeholder="item.value.placeholder"
                            placeholder-class="placeholder color-tip" v-model="item.val"></textarea></view>
                </view>
            </view>

            <picker mode="selector" :range="item.value.options" v-if="item.controller == 'Select'"
                    ...
                    <view class="box check-group-box">
                        <checkbox-group @change="checkboxChange($event, index)">
                            <label v-for="(v, k) in item.option_lists" :key="k">
                                ...
                                <view class="checkbox">
                                    <text class="iconfont"
                                        :class="{ 'icon-fuxuankuang2': !v.checked, 'icon-fuxuankuang1 color-base-text': v.checked }"></text>
                                    {{ v.value }}
                                </view>
                            </label>
                        </checkbox-group>
                        ...
                        <radio-group @change="radioChange($event, index)">
                            <label v-for="(v, k) in item.option_lists" :key="k">
                                ...
                                <view class="radio-box">
                                    <text class="iconfont"
                                        :class="{ 'icon-yuan_checkbox': item.val != v.value, 'icon-yuan_checked color-base-text': item.val == v.value }"></text>
                                    {{ v.value }}
                                </view>
                            </label>
                        </radio-group>
                ...
                <view class="order-cell">
                    ...
                    <view class="box box-flex">
                        <picker mode="date" :value="item.val" @change="bindDateChange($event, index)">
                            <view class="uni-input" :class="{ 'color-tip': !item.val }">
                                {{ item.val ? item.val : item.value.placeholder }}</view>
                        </picker>
                    </view>
                    <text class="iconfont icon-right"></text>
                </view>
                    ...
                    <view class="box date-boxs">
                        <view class="date-box">
                            <picker mode="date" :value="item.start_date" @change="bindStartDateChange($event, index)">
                                <view class="picker-box">
                                    <view class="uni-input" :class="{ 'color-tip': !item.start_date }">
                                        {{ item.start_date ? item.start_date : item.value.placeholder_start }}</view>
                                </view>
                            </picker>
                        </view>
                        ...
                        <view class="date-box">
                            <picker mode="date" :value="item.end_date" @change="bindEndDateChange($event, index)">
                                <view class="picker-box">
                                    <view class="uni-input" :class="{ 'color-tip': !item.end_date }">
                                        {{ item.end_date ? item.end_date : item.value.placeholder_end }}</view>
                                </view>
                            </picker>
                        </view>
                    </view>
                    ...
                    <view class="box date-boxs">
                        <view class="date-box">
                            <picker mode="time" :value="item.start_time" @change="bindStartTimeChange($event, index)">
                                <view class="picker-box">
                                    <view class="uni-input" :class="{ 'color-tip': !item.start_time }">
                                        {{ item.start_time ? item.start_time : item.value.placeholder_start }}</view>
                                </view>
                            </picker>
                        </view>
                        <view class="interval iconfont icon-jian"></view>
                        <view class="date-box">
                            <picker mode="time" :value="item.end_time" @change="bindEndTimeChange($event, index)">
                                <view class="picker-box">
                                    <view class="uni-input" :class="{ 'color-tip': !item.end_time }">
                                        {{ item.end_time ? item.end_time : item.value.placeholder_end }}</view>
                                </view>
                            </picker>
                        </view>
                    </view>
            ...
            <view class="order-wrap" v-if="item.controller == 'City'">
                <view class="order-cell box-flex">
                    ...
                    <view class="box">
                        <pick-regions :default-regions="item.default_regions" :select-arr="item.select_arr"
    ...
    export default {
        ...
        methods: {
            ...
            verify() {
                ...
                for (let i = 0; i < this.formData.length; i++) {
                    ...
                    if (item.controller == 'Datelimit') {
                        ...
                        if (this.$util.timeTurnTimeStamp(item.start_date) > this.$util.timeTurnTimeStamp(item
                            .end_date)) {
                            verify = false;
                            this.$util.showToast({
                                title: '结束日期不能小于开始日期'
            ...
            uploadImg(index) {
                this.$util.upload(
                    ...
                    res => {
                        if (res.length > 0) {
                            res.forEach(v => {
                                if (this.formData[index].img_lists.length >= Number(this.formData[index]
                                        .value.max_count)) {
                                    this.$util.showToast({
            ...
            bindStartDateChange(e, index) {
                ...
                this.$set(this.formData[index], 'val', this.formData[index].start_date + ' - ' + this.formData[index]
                    .end_date);
                this.$forceUpdate();
            },
