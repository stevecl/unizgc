<template>
	...
	<view class="guafen-box">
		<view v-if="groupInfoList.status != 2">
			<view class="guafen_adv"
				:style="{ backgroundImage: 'url(' + $util.img('public/uniapp/divideticket/guafen_bg.png') + ')' }">
				<view class="explain">
				...
				<view class="guafen_content" v-if="groupInfoList == '' && guafenInfo.status == 1"
				...
				<view class="guafen_content"
					...
					<view class="guafen_photo">
						<scroll-view :scroll-y="true" :show-scrollbar="false"
							v-if="groupInfoList.member_list.length > 0">
							<view class="guafen-lists">
								<view class="guafen_header_img"
									v-for="(headerItem, headerIndex) in groupInfoList.member_list"
									:key="headerItem.member_id">
									<image v-if="headerItem.headimg != ''" :src="$util.img(headerItem.headimg)" mode=""
										@error="headerimageError(headerIndex)"></image>
									<image v-else :src="$util.img('public/uniapp/default_img/head.png')" mode="">
									</image>
								</view>
								<view class="guafen_header_img" @click="openSharePopup"
									v-for="(itm, inx) in groupInfoList.num - groupInfoList.member_list.length"
									:key="inx">
									<text>+</text>
								</view>
							</view>
					...
					<view class="guafen_content_title">
						仅差
						<text
							class="guafen_content_color">{{ groupInfoList.num - groupInfoList.member_list.length }}</text>
						人，即可瓜分
					...
					<view class="guafen_content_time">
						...
						<uni-count-down class="guafen_content_color" :hour="seckillH" :minute="seckillI"
							:second="seckillS" borderColor="#fff" color="#FF4544" />
				...
				<block v-if="groupInfoList != '' && groupInfoList.status == 1 && g_member_state == 0">
					<view class="guafen_content-one"
						:style="{ 'background-image': 'url(' + $util.img('public/uniapp/divideticket/guafen_success_bg.png') + ')' }">
						<view class="guafen-success">恭喜你，成功获得优惠券</view>
						<view class="guafen_photo"
							:class="groupInfoList.member_list.length > 4 ? 'guafen-lists' : 'guafen-lists-two'">
							<scroll-view :scroll-y="true" :show-scrollbar="false"
								v-if="groupInfoList.member_list.length > 0">
								<view
									:class="groupInfoList.member_list.length > 4 ? 'guafen-lists' : 'guafen-lists-two'">
									<view class="guafen_header_img"
										v-for="(headerItem, headerIndex) in groupInfoList.member_list"
										:key="headerItem.member_id">
										<image v-if="headerItem.headimg != ''" :src="$util.img(headerItem.headimg)"
											mode="" @error="headerimageError(headerIndex)"></image>
										<image v-else :src="$util.img('public/uniapp/default_img/head.png')" mode="">
										</image>
									</view>
								</view>
					...
					<scroll-view scroll-y="true" class="rule">
						<view>
							...
							<view class="text">{{ $util.timeStampTurnTime(guafenInfo.start_time) }} -
								{{ $util.timeStampTurnTime(guafenInfo.end_time) }}</view>
			...
			<uni-popup ref="linkPopup" type="center" :maskClick="false">
				<view class="warp-popup" v-if="inviterInfo">
					<view class="reward-wrap"
						:style="{ backgroundImage: 'url(' + $util.img('public/uniapp/divideticket/guafen_popup.png') + ')' }">
						<image
							:src="$util.img(inviterInfo.headimg ? $util.img(inviterInfo.headimg) : $util.getDefaultImage().head)"
							@error="inviterInfo.headimg = $util.getDefaultImage().head" class="img-head"></image>
				...
				<view class="warp-popup" v-else>
					<view class="reward-wrap"
						:style="{ backgroundImage: 'url(' + $util.img('public/uniapp/divideticket/guafen_popup.png') + ')' }">
						<image
							:src="$util.img(groupInfoList.header_headimg ? $util.img(groupInfoList.header_headimg) : $util.getDefaultImage().head)"
							@error="groupInfoList.header_headimg = $util.getDefaultImage().head" class="img-head">
						</image>
		...
		<view @touchmove.prevent.stop class="reward-popup-two">
			<uni-popup ref="linkSuccessPopup" type="center" :maskClick="false">
				<view class="reward-wrap-two"
					:style="{ backgroundImage: 'url(' + $util.img('public/uniapp/divideticket/guafen_success.png') + ')' }">
					<view class="txt-two">
...
<script>
	...
	export default {
		...
		methods: {
			...
			join() {
				if (!uni.getStorageSync('token')) {
					setTimeout(() => {
						this.$refs.login.open('/pages_tool/hongbao/index?coupon_id=' + this.couponId +
			...
			getLaunch() {
				if (!uni.getStorageSync('token')) {
					setTimeout(() => {
						this.$refs.login.open('/pages_promotion/divideticket/index?coupon_id=' + this.couponId +
							'&group_id=' + this.groupId);
					});
				} else {
					this.$api.sendRequest({
			...
			copyUrl() {
				...
				if (this.couponId) text += '?coupon_id=' + this.couponId + '&group_id=' + this.groupInfoList.group_id +
					'&inviter_id=' + this.guafenInfo.member_id;
				this.$util.copy(text, () => {
			...
			setPublicShare() {
				let shareUrl =
					this.$config.h5Domain +
					'/pages_promotion/divideticket/index?cid=' +
					this.guafenInfo.coupon_id +
					'&gid=' +
					this.groupInfoList.group_id +
					'&id=' +
					this.guafenInfo.member_id;
		...
		onShareAppMessage(res) {
			var path = '/pages_promotion/divideticket/index?cid=' + this.guafenInfo.coupon_id + '&gid=' + this
				.groupInfoList.group_id + '&id=' + this.guafenInfo.member_id;
		...
		onShareTimeline() {
			...
			let query = (query = '?cid=' + this.guafenInfo.coupon_id + '&gid=' + this.groupInfoList.group_id + '&id=' +
				this.guafenInfo.member_id);
