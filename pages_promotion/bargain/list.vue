						...
						<view class="goods-item margin-bottom" v-for="(item, index) in dataListing" :key="index">
							<view class="goods-item-content">
								<view class="goods-img" @click="toDetailP(item)">
									...
									<view class="color-base-bg goods-tag" v-if="goodsTag(item) != ''">
										{{ goodsTag(item) }}
									</view>
								</view>
									...
									<view class="info-bottom">
										<view class="price-box">
											<view class="discount-price">
												已砍
												<text
													class="price price-font">{{ (item.price - item.curr_price).toFixed(2) }}</text>
												，
											</view>
											<view class="delete-price ">
												还剩
												<text
													class="price price-font">{{ $lang('common.currencySymbol') }}{{ (item.curr_price - item.floor_price).toFixed(2) }}</text>
											</view>
										</view>
							...
							<view class="goods-item-bottom">
								<view class="item-bottom-left">
									<uni-count-down :day="item.time.d" :hour="item.time.h" :minute="item.time.i"
										:second="item.time.s" color="#fff" borderColor="none" splitorColor="#303133"
										backgroundColor="#303133" />
									<view class="color-sub txt">后结束</view>
								</view>
								<view class="item-bottom-right"><button type="primary" @click="toDetailP(item)"
										size="mini" class="mini">继续砍价</button></view>
							</view>
						</view>
					</view>
								...
								<view class="info-wrap">
									<view class="name-wrap">
										...
										<view class="progress-wrap">
											...
											<view class="progress-point"></view>
											<view class="progress-select icon"
												:style="{ left: 'calc(' + progress(item) + '% - 8rpx)' }">
												<text class="iconfont icon-futou"></text>
											</view>
										</view>
									</view>
							...
							<view class="goods-item-bottom">
								<view class="item-bottom-left">
									...
									<text class="price price-style large">
										{{
											parseFloat(item.floor_price)
												.toFixed(2)
												.split('.')[0]
										}}
									</text>
									<text class="unit price-style large">
										.{{
											parseFloat(item.floor_price)
												.toFixed(2)
												.split('.')[1]
									...
									<view class="delete-pirce">{{ $lang('common.currencySymbol') }}{{ item.price }}
									</view>
								</view>
								<view class="item-bottom-right"><button type="primary" @click="toDetail(item)"
										size="mini" class="mini">去砍价</button></view>
							</view>
						</view>
					</view>
				</block>
...
<script>
	...
	export default {
		...
		methods: {
				...
				this.$api.sendRequest({
					...
					success: res => {
						...
						for (var index in res.data) {
							this.dataListing[index].time = this.$util.countDown(res.data[index].end_time - res
								.timestamp);
						}
						this.isLoading = true;
						if (this.$refs.loadingCover) this.$refs.loadingCover.hide();
			...
			progress(data) {
				let progress = ((parseInt(data.sale_num) / (parseInt(data.bargain_stock) + parseInt(data.sale_num))) * 100)
					.toFixed();
				if (progress == 'NaN') {
					progress = 0;
				}
			...
			progressP(data) {
				let progress = (((parseFloat(data.price) - parseFloat(data.curr_price)) / parseFloat(data.price)) * 100)
					.toFixed();
				if (progress == 'NaN') {
					progress = 0;
				}
