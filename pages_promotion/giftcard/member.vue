	...
	<view class="page">
			...
			<view class="member-info" v-if="memberInfo">
				<view class="member-headimg">
					<image :src="memberInfo.headimg ? $util.img(memberInfo.headimg) : $util.getDefaultImage().head"
						mode="widthFix" @error="memberInfo.headimg = $util.getDefaultImage().head"></image>
				</view>
		...
		<view class="member-box" v-if="memberInfo">
			<view class="member-action">
				<view class="action-item" @click="redirectTo('/pages_promotion/giftcard/receive_list')"
					:style="{'background-image' : 'url('+$util.img('public/uniapp/giftcard/icon-card-yellow.png')+') '}">
					<view class="title">收到的卡片</view>
					<view class="desc">可赠送他人</view>
				</view>
				<view class="action-item" @click="redirectTo('/pages_promotion/giftcard/give_list')"
					:style="{'background-image' : 'url('+$util.img('public/uniapp/giftcard/icon-card-red.png')+') '}">
				...
				this.$api.sendRequest({
					...
					success: res => {
						if (res.code == 0) {
							...
							if (this.memberInfo && this.memberInfo.mobile) this.memberInfo.mobile = this
								.mobileHide(this.memberInfo.mobile)
						}
					}
				});
			},
