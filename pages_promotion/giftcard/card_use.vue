		...
		<view class="order-container" :class="{ 'safe-area': isIphoneX }">
			...
			<scroll-view scroll-y="true" class="order-scroll-container">
				...
				<template v-if="paymentData">
						...
						<view class="address-box" :class="{ 'not-delivery-type': goodsData.express_type.length <= 1 }"
							...
							<view v-if="localMemberAddress">
								<block v-if="storeList && Object.keys(storeList).length > 1">
									<view class="local-delivery-store" v-if="storeInfo"
									...
									<view class="store-address-info">
										<view class="info-wrap">
											...
											<view class="store-detail">
												...
												<view class="address">{{ storeInfo.full_address }}
													{{ storeInfo.address }}
												</view>
											</view>
										</view>
								...
								<view class="mobile-wrap store-mobile">
									<view class="form-group">
										...
										<input type="text" placeholder-class="color-tip placeholder" class="input"
											disabled v-model="orderCreateData.member_address.name" />
									</view>
								</view>
								<view class="mobile-wrap store-mobile">
									<view class="form-group">
							...
							<view v-for="(goodsItem, goodsIndex) in goodsData.goods_list" :key="goodsIndex">
								...
								<view class="goods-form" v-if="goodsItem.goods_form" @click="editForm(goodsIndex)">
									<ns-form :data="goodsItem.goods_form.json_data" ref="goodsForm"
										:custom-attr="{ sku_id: goodsItem.sku_id, form_id: goodsItem.goods_form.id }"></ns-form>
									<text class="cell-more iconfont icon-right"></text>
					...
					<view v-if="paymentData.system_form" class="system-form-wrap"><ns-form
							:data="paymentData.system_form.json_data" ref="form"></ns-form></view>

					<!-- 订单金额 -->
					<template v-if="calculateData">
						<view class="order-submit bottom-safe-area">
							<view class="submit-btn"><button type="primary" class="sava-btn mini" size="mini"
									@click="create()">立即提货</button></view>
						</view>
					...
					<uni-popup ref="buyerMessagePopup" type="bottom">
						<view style="height: auto;" class="buyermessag-popup popup" @touchmove.prevent.stop>
							...
							<view class="popup-footer" @click="saveBuyerMessage"
						...
						if (res.code == 0 && res.data) {
							...
							if (data.shop_goods_list && data.shop_goods_list.express_type && data
								...
								if (
									...
									uni.getStorageSync('deliveryTime')['delivery_type'] == this.orderCreateData
							...
							// 优惠券
							if (data.shop_goods_list && data.shop_goods_list.coupon_list && data
								.shop_goods_list.coupon_list[0])
