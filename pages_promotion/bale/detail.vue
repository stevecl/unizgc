	...
	<view class="container">
		<block v-if="info">
			...
			<view class="goods-wrap">
				<view class="goods-item" v-for="(item, index) in goodsList" :key="index">
					<view class="goods-image" @click="$util.redirectTo('/pages/goods/detail', { sku_id: item.sku_id })">
						<image :src="$util.img(item.sku_image, { size: 'mid' })" mode="widthFix"
					...
					<view class="goods-info">
						...
						<view class="goods-bottom">
							...
							<view class="num">
								<block
									v-if="cart['goods_' + item.goods_id] == undefined || !cart['goods_' + item.goods_id]['sku_' + item.sku_id]">
									<view class="num-wrap">
										<text v-if="item.stock>0" class="iconfont icon-add-fill color-base-text"
											@click="singleSkuPlus(item)"></text>
								...
								<block v-else-if="cart['goods_' + item.goods_id]['sku_' + item.sku_id]">
									<view class="num-wrap">
										...
										<text
											class="goods-num">{{ cart['goods_' + item.goods_id]['sku_' + item.sku_id].num }}</text>
										<text class="iconfont icon-add-fill color-base-text"
											@click="singleSkuPlus(item)"></text>
									</view>
								</block>
							</view>
			...
			<view class="footer-wrap">
				...
				<view class="right">
					<view class="sub-btn color-base-bg"
						v-if="totalNum > 0 && totalNum % info.num == 0 && info.status != 0" @click="submit">立即下单</view>
		...
		<ns-goods-sku v-if="goodsSkuDetail" ref="goodsSku" @refresh="refreshGoodsSkuDetail" @confirm="joinCart"
			:goodsId="goodsSkuDetail.goods_id" :goods-detail="goodsSkuDetail"></ns-goods-sku>
