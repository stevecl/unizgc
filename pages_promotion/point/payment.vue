	...
	<view class="order-container" :class="{ 'safe-area': isIphoneX }">
		...
		<scroll-view scroll-y="true" class="order-scroll-container">
			...
			<template v-if="orderPaymentData.exchange_info.type == 1 && orderPaymentData.is_virtual == 0">
				<!-- 配送方式 -->
				<view class="delivery-mode" v-if="orderPaymentData.express_type.length > 1 && orderCreateData.delivery">
					<view class="action">
				...
				<view class="address-box" :class="{'not-delivery-type': orderPaymentData.express_type.length <= 1}"
					...
					<view class="info-wrap" :class="{'local': orderPaymentData.delivery.delivery_type == 'local'}"
						v-if="orderPaymentData.member_address" @click="selectAddress">
						<view class="content">
							<text
								class="name font-size-base">{{ orderPaymentData.member_address.name ? orderPaymentData.member_address.name : '' }}</text>
							<text
								class="font-size-base mobile">{{ orderPaymentData.member_address.mobile ? orderPaymentData.member_address.mobile : '' }}</text>
					...
					<block v-if="orderPaymentData.delivery.delivery_type == 'local'">
						<view class="local-box"
							v-if="orderPaymentData.local_config && orderPaymentData.local_config.info && orderPaymentData.local_config.info.time_is_open == 1">
							<view class="pick-block" @click="localtime">
								<view class="font-size-base">送达时间</view>
								<view class="time-picker">
									<text
										:class="{'color-tip': !orderCreateData.buyer_ask_delivery_title}">{{ orderCreateData.buyer_ask_delivery_title ? orderCreateData.buyer_ask_delivery_title : '请选择送达时间' }}</text>
				...
				<view class="store-box" :class="{'not-delivery-type': orderPaymentData.express_type.length <= 1}"
						...
						<block v-if="orderPaymentData.exchange_info.type == 2">
							<view class="goods-img">
								<image
									...
									@error="imageError()" mode="aspectFill"></image>
							</view>
						</block>
