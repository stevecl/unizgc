			...
			<view class="card-info" v-if="cardDetail.card_item.length">
				...
				<view class="card-content">
					<view class="card-item" v-for="(item,index) in cardDetail.card_item"
						...
						<button type="default" v-if="cardDetail.card_type == 'commoncard'&&cardDetail.total_num || cardDetail.card_type != 'commoncard'&&(item.num == 0 || (item.num-item.use_num))" :class="['button', {'charge-off': item.goods_class == 4},{'pick-goods': item.goods_class == 1}]" @click.stop="toUseFn(item)">{{item.goods_class == 4&&'去核销' || item.goods_class == 1&&'去提货'}}</button>
					</view>
				</view>
