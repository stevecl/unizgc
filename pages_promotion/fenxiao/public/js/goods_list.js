...
export default {
	...
	methods: {
		...
		getMemberInfo() {
			this.$api.sendRequest({
				...
				success: res => {
					if (res.code == 0) {
						this.is_fenxiao = Boolean(res.data.is_fenxiao);
						this.memberId = res.data.member_id; 
					}else {
						this.is_fenxiao = false;
					}
				}
			});
		},
		...
		copyUrl() {
			let text = this.$config.h5Domain + '/pages/goods/detail?goods_id=' + this.goodsList[this.currIndex].goods_id + '&source_member=' + this.memberId;
			this.$util.copy(text, () => {
	...
	onShareAppMessage(res) {
		...
		if (this.shareType == "goods") {
			path = `/pages/goods/detail?goods_id=${this.goodsList[this.currIndex].goods_id}&sku_id=${this.goodsList[this.currIndex].sku_id}`;
