<template>
	...
	<view class="page" :style="{ background: bgColor }">
		<view v-if="show">
			<mescroll-uni @getData="getData" ref="mescroll" :size="10" v-if="seckillId && addonIsExist.seckill"
				@scroll="scroll">
				<block slot="list">
					<ns-adv keyword="NS_SECKILL" class="adv-wrap"></ns-adv>
					<view class="time-wrap">
						<scroll-view :scroll-x="true" class="scroll-wrap" scroll-with-animation
							:scroll-into-view="'a' + seckillId">
							<view class="time-item" v-for="(item, key) in timeList" :key="key" :id="'a' + item.id"
								@click="ontabtap(item.id, key)" :class="seckillId == item.id ? 'active' : ''">
					...
					<view class="goods-list single-column" v-if="dataList.length">
						<view class="item" v-for="(item, goodsIndex) in dataList" :key="goodsIndex"
							@click="toGoodsDetail(item)">
							<view class="img-wrap">
								<image :src="$util.img(item.goods_image, { size: 'mid' })" mode="widthFix"
									@error="imageError(goodsIndex)"></image>
							</view>
							<view class="content">
								...
								<view class="bottom-wrap">
									<view class="price-wrap">
										<view class="discount-price">
											...
											<text
												class="price price-style large">{{ item.seckill_price.split('.')[0] }}</text>
											<text
												class="unit price-style small">{{ '.' + item.seckill_price.split('.')[1] }}</text>
										</view>
