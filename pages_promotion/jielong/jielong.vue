<template>
	...
	<view class="body">
		<view class="topHead"
			:style="{ backgroundImage: 'url(' + $util.img('public/uniapp/jielong/banyuan.png') + ')' }">
			<view class="jielong-head">
				<view class="countdown" v-if="status == 0">
					距活动开始仅剩：
					<uni-count-down :day="time.d" :hour="time.h" :minute="time.i" :second="time.s" color="#fff"
						splitorColor="#FF4644" backgroundColor="#FF4644" />
				</view>
					...
					<view class="content">
						...
						<view class="content-num" v-if="item.member_headimg.length > 0">
							<view class="content-num-image">
								<image :src="$util.img(data) ? $util.img(data) : $util.getDefaultImage().head"
									v-for="(data, value) in item.member_headimg.slice(0, 3)" :key="value"></image>
							</view>
							<text :class="item.member_num == 2 ? 'text-left' : ''"
								v-if="item.member_num < 3">{{ item.member_num }}人已买</text>
						...
						<view class="content-bnutton">
							<view v-if="item.goods_spec_format && status != 2" @click="codeView(item.sku_id)"
								class="color-base-bg">
								选规格
							...
							<view v-else-if="item.goods_spec_format && status == 2"
								class="color-base-bg color-base-bgHover">选规格</view>
							<block v-else>
								<view class="content-bnutton-body" v-if="data.sku_id == item.sku_id"
									v-for="(data, value) in shoplist" :key="value">
									<view class="content-bnutton-sum iconfont icon-jianshao content-bnutton-left"
										@click="singleSkuReduce(item)"></view>
									<view class="content-bnutton-center">{{ data.num }}</view>
								</view>
								<view v-if="status != 2"
									class="content-bnutton-sum iconfont icon-add-fill content-bnutton-right"
									@click="singleSkuPlus(item)"></view>
								<view v-else
									class="content-bnutton-sum iconfont icon-add-fill content-bnutton-right content-bnutton-rightNew">
								</view>
							</block>
						</view>
					</view>
		...
		<view class="old-buy" v-else>
			...
			<view :class="seeMores ? 'old-buy-head-content' : ''">
				<view class="content" v-for="(item, index) in (jielong_buy_page.list || '').slice(0, cutting)"
					:key="index">
					<view style="display: flex;">
						<view class="old-buy-content">
							<image class="old-buy-content-image"
								:src="item.headimg == '' ? $util.getDefaultImage().head : $util.img(item.headimg)">
							</image>
		...
		<view :class="zIndex ? 'cart-bottom zIndex' : 'cart-bottom'">
			<view class="cart-bottom-content">
				<view class="cart-bottom-gouwu" @click="CartPopup">
					<view
						:class="status == 1 && shoplist.length > 0 ? 'cart-bottom-gouwu-content iconhover' : 'cart-bottom-gouwu-content'">
						<view class="cart-bottom-gouwu-content-sum" v-if="status == 1 && shoplist.length > 0">{{ num }}
						</view>
			...
			<view
				:class="status == 1 && shoplist.length > 0 ? 'cart-gouwu-tijiao ' : 'cart-gouwu-tijiao cart-gouwu-tijiaohover'"
				v-if="status == 1" @click="settlement">
				接龙购买
			</view>
						...
						<view class="content">
							...
							<view class="content-bnutton">
								<block>
									...
									<view class="content-bnutton-sum iconfont icon-add-fill content-bnutton-right"
										@click="singleSkuPlus(item)"></view>
								</block>
							</view>
						</view>
				...
				<view class="popupContent-top">
					<view class="popupContent-top-title">
						<view class="popupContent-top-title-left">
							<image :src="switchImage ? specifications.sku_image : $util.img(specifications.sku_image)"
								mode=""></image>
						</view>
						<view class="popupContent-top-title-right">
							...
							<view class="popupContent-top-title-right-textBottom" v-if="specifications.stock_show">
								库存{{ specifications.stock }}件</view>
						</view>
					</view>
					<view class="popupContent-top-center">
						<view class="popupContent-top-center-content" v-for="(items, indexs) in specificationsItem"
							:key="indexs">
							<view class="popupContent-top-center-content-text">{{ items.spec_name }}</view>
							<view class="popupContent-top-center-content-small">
								<view @click="switchs(item.sku_id)"
									:class="specifications.sku_id == item.sku_id ? 'small-content small-content-hover' : 'small-content'"
									v-for="(item, i) in items.value" :key="i">
					...
					<view class="popupContent-top-bottom">
						...
						<view class="popupContent-top-bottom-content">
							...
							<view v-if="display" class="popupContent-top-bottom-content-sum">
								<view class="content-bnutton-sum iconfont icon-jianshao content-bnutton-left"
									@click="singleSkuReduce(specifications)"></view>
								<view v-if="item.sku_id == specifications.sku_id" class="content-bnutton-center"
									v-for="(item, index) in shoplist" :key="index">
									{{ item.num }}
								</view>
								<view class="content-bnutton-sum iconfont icon-add-fill content-bnutton-right"
									@click="singleSkuPlus(specifications)"></view>
							</view>
							<view v-else>
								<button type="primary" class="popupContent-top-bottom-content-right"
									v-if="specifications.stock && specifications.stock != 0"
									@click="shoppingCart(specifications)">
									加入购物车
								</button>
								<button type="primary" class="popupContent-top-bottom-content-right" disabled="true"
									v-else>确定</button>
							</view>
						</view>
					</view>
		...
		<uni-popup ref="share" type="center" class="share-img">
			<view class="share-image">
				<image :src="$util.img(shareImage)" :show-menu-by-longpress="true"></image>
			</view>
		</uni-popup>
				...
				this.$api.sendRequest({
					...
					success: res => {
						if (res.code == 0 && res.data && res.data.info) {
							...
							} else if (this.jielong_info.start_time < this.timer && this.jielong_info
								.end_time > this.timer) {
					...
					success: res => {
						...
						if (data.length > 0) {
							data.forEach(item => {
								...
								this.money = parseFloat((parseFloat(item.price) * parseInt(item.num) +
										this.money))
									.toFixed(2);
							});
						}
