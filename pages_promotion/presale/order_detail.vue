<template>
	...
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }">
				...
				<view class="status-name">
					...
					<block v-else>
						...
						<block v-if="orderData.order_status == 1">
							<view class="desc" v-if="orderData.pay_start_time > timestamp">
								尾款开始支付时间：{{ $util.timeStampTurnTime(orderData.pay_start_time) }}</view>
							<view class="desc"
								v-else="orderData.pay_start_time < timestamp && orderData.pay_end_time > timestamp">
								尾款支付结束时间：{{ $util.timeStampTurnTime(orderData.pay_end_time) }}</view>
						</block>
					</block>
				</view>
			</view>
						...
						<view class="goods-sub-section">
							<view>
								<text class="goods-price ">
									...
									<text
										class="font-size-base price-style large">{{ parseFloat(orderData.price).toFixed(2).split(".")[0] }}</text>
									<text
										class="unit price-style small">.{{ parseFloat(orderData.price).toFixed(2).split(".")[1] }}</text>
								</text>
							</view>
							<view>
		...
		<view class="order-money presale-process">
			...
			<view class="order-cell">
				<view class="tit"
					:class="{'color-base-text': orderData.pay_deposit_time > 0 && orderData.pay_final_time >= 0}">
			...
			<view class="order-action" :class="{ 'bottom-safe-area': isIphoneX }" v-if="orderData.action.length > 0">
				<block v-for="(operationItem, operationIndex) in orderData.action" :key="operationIndex">
					<block v-if="operationItem.action == 'orderPayFinal'">
						<block
							v-if="(orderData.refund_status == 0 || orderData.refund_status == -1) &&  orderData.order_status != -1">
							<view class="order-box-btn color-base-border color-base-text"
								@click="operation(operationItem.action)"
								v-if="orderData.pay_start_time < timestamp && orderData.pay_end_time > timestamp">
								{{ operationItem.title }}</view>
					...
					<block v-else-if="operationItem.action == 'refundDeposit'">
						<view class="order-box-btn"
							v-if="(orderData.refund_status == 0 && orderData.is_deposit_back == 0) || (orderData.refund_status == -1 && orderData.is_deposit_back == 0)"
							@click="operation(operationItem.action)">{{ operationItem.title }}</view>
					</block>
		...
		<!-- 选择支付方式弹窗 -->
		<ns-payment ref="choosePaymentPopup" :payMoney="payMoney" @confirm="pay" :balanceDeduct="balanceDeduct"
			:isBalance="isBalance" @useBalance="useBalance"></ns-payment>
...
<script>
	...
	export default {
		...
		computed: {
			...
			balanceDeduct() {
				if (this.orderData.order_status == 1 && this.orderData.final_out_trade_no == '' && this.memberBalance >
					0) {
			...
			presaleDiscount() {
				return (parseFloat(this.orderData.presale_money) - parseFloat(this.orderData.presale_deposit_money))
					.toFixed(2);
			}
		},
		onShow() {
		...
		methods: {
			getMmemberBalance() {
				this.$api.sendRequest({
					...
					success: res => {
						if (res.data) {
							this.memberBalance = parseFloat(res.data.balance) + parseFloat(res.data
								.balance_money);
						}
					}
				});
