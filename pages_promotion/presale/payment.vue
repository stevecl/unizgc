	...
	<view class="order-container" :class="{ 'safe-area': isIphoneX }">
		...
		<scroll-view scroll-y="true" class="order-scroll-container">
				...
				<view class="address-box"
					...
					<block v-if="orderPaymentData.delivery.delivery_type == 'local'">
						...
						<view class="info-wrap local" v-if="localMemberAddress" @click="selectAddress">
							<view class="content">
								<text
									class="name font-size-base">{{ localMemberAddress.name ? localMemberAddress.name : '' }}</text>
								<text
									class="font-size-base mobile">{{ localMemberAddress.mobile ? localMemberAddress.mobile : '' }}</text>
					...
					<block v-if="orderPaymentData.delivery.delivery_type == 'express'">
						<view class="info-wrap" v-if="memberAddress" @click="selectAddress">
							<view class="content">
								<text
									class="name font-size-base">{{ memberAddress.name ? memberAddress.name : '' }}</text>
								<text
									class="font-size-base mobile">{{ memberAddress.mobile ? memberAddress.mobile : '' }}</text>
								<text class="cell-more iconfont icon-right"></text>
								<view class="desc-wrap">
					...
					<view class="goods-item"
						...
						<view class="goods-wrap">
							...
							<view class="goods-info">
								<view class="top-wrap">
									<view @click="navigateTo(goodsItem.goods_id)" class="goods-name">
										{{ goodsItem.sku_name }}
									</view>
			...
			<view class="order-money">
				...
				<view class="order-cell"
					v-if="orderCreateData.is_invoice && orderPaymentData.invoice_delivery_money > 0">
				...
				<view class="order-cell"
					...
					<view class="box align-right">
						<text class="ns-text-color-black color-base-text">
							...
							<text
								class="font-size-base">{{ orderPaymentData.presale_deposit_money | moneyFormat }}</text>
						</text>
					</view>
				</view>
				<view class="order-cell">
			...
			<view class="error-message" v-if="
					orderPaymentData.delivery &&
						...
						orderPaymentData.shop_goods_list.delivery &&
						orderPaymentData.shop_goods_list.delivery.error &&
						orderPaymentData.shop_goods_list.delivery.error !== ''
				">
				{{ orderPaymentData.shop_goods_list.delivery.error_msg }}
			</view>
					...
					<button v-else class="no-submit mini" size="mini">
						<block v-if="
								orderPaymentData.delivery &&
									...
									orderPaymentData.shop_goods_list.delivery &&
									orderPaymentData.shop_goods_list.delivery.error &&
									orderPaymentData.shop_goods_list.delivery.start_money > orderPaymentData.presale_deposit_money
							">
						...
						<view class="buyermessag-cell">
							<view class="buyermessag-form-group">
								<textarea type="text" maxlength="100" placeholder="留言前建议先与商家协调一致"
									placeholder-class="color-tip" v-model="orderCreateData.buyer_message"></textarea>
							</view>
						</view>
					</view>
				</scroll-view>
				...
				<scroll-view scroll-y="true" class="popup-body" :class="{ 'safe-area': isIphoneX }">
					<view class="coupon-item"
						...
						<view class="coupon-info">
							...
							<view class="iconfont"
								:class="orderPaymentData.coupon.coupon_id == couponItem.coupon_id ? 'icon-yuan_checked color-base-text' : 'icon-yuan_checkbox'">
							</view>
						</view>
					</view>
				</scroll-view>
