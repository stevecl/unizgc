<template>
	...
	<view :class="isIphoneX ? 'iphone-x' : ''">
			...
			<block slot="list">
				<view class="coupon-listone">
					<view class="item" :class="['item',item.state != 1&&'item-disabled']" v-for="(item, index) in list"
						...
						<view class="item-btn">
							<view class="tag" style="margin-bottom: 20rpx;" @click.stop="showTicketCodePop(item.coupon_code)">券码</view>
		...
		<!-- 券码弹窗 -->
		<uni-popup ref="ticketCodePopup" class="ticket-code-pop">
			<view style="position: relative; padding: 20rpx 40rpx;">
				<view class="ticket-code-text" style="margin-bottom: 20rpx;">请出示二维码进行核销</view>
				<canvas canvas-id="ticketCode" class="ticketCode"></canvas>
				<view class="ticket-code-text">{{ currentTicketCode }}</view>
				<view class="ticket-code-close" @click.stop="$refs.ticketCodePopup.close()">X</view>
				<!-- <view class="ticket-code-footer" @click="$refs.ticketCodePopup.close()">确定</view> -->
			</view>
		</uni-popup>
	</view>
</template>
...
import uqrcode from '@/common/js/uqrcode.js'
	export default {
		...
		methods: {
			// 查看券码
			showTicketCodePop(text) {
				this.$refs.ticketCodePopup.open();
				this.currentTicketCode = text
				uqrcode.make({
					canvasId: 'ticketCode',
				...
				this.$api.sendRequest({
					url: '/prod-api/system/order/couponTb?couponCode=' + text,
					success: res => {}
				});
			},
			//切换状态
...
<style lang="less">
	.ticket-code-pop .uni-popup__wrapper-box {
