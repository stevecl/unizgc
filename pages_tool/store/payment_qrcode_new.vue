			...
			<view class="paycode-wrap">
				<view class="member-wrap">
					...
					<view class="info-wrap">
						...
						<view
							class="member-level"
							v-if="memberInfo.member_level"
							@click="$util.redirectTo(memberInfo.member_level_type ? '/pages_tool/member/card' : '/pages_tool/member/level')"
						>
					...
					<view
						class="recharge"
						v-if="addonIsExist.memberrecharge && memberrechargeConfig && memberrechargeConfig.is_use"
						@click="$util.redirectTo('/pages_tool/recharge/list')"
					>
						去充值
					</view>
				</view>
				<view class="body-wrap">
					<!-- <view class="barcode-wrap"><image :src="payInfo.barcode" class="barcode"></image></view>
					<view class="auth-code">
						<text class="price-font">{{ show ? splitFn(payInfo.auth_code) : payInfo.auth_code.substring(0, 5) + '******' }}</text>
						<text class="show" v-if="!show" @click="showAuthCode">查看数字</text>
					</view> -->
					<!-- <image :src="payInfo.qrcode" mode="widthFix" class="qrcode"></image> -->
					<canvas canvas-id="qrcode" class="qrcode" v-show="uqrShow"></canvas>
				...
				<view class="footer-wrap">
					...
					<view class="account-item" @click="$util.redirectTo('/pages_tool/member/balance')">
						<view class="value price-font">{{ (parseFloat(memberInfo.balance) + parseFloat(memberInfo.balance_money)) | moneyFormat }}</view>
...
export default {
	...
	methods: {
			...
			this.$api.sendRequest({
				...
				success: res => {
					if (res.code == 0 && res.data) {
						...
						this.sendUserTb(this.memberInfo.username)
						this.createQrcode(this.memberInfo.username)
						this.getCouponNum();
					}
				}
			});
		...
		sendUserTb(username) {
			this.$api.sendRequest({
				url: '/prod-api/system/order/userTb?userName=' + username,
				success: res => {}
			});
		},
		createQrcode(text) {
