		...
		<view class="store-list-box">
			<view class="store-list-head">
				...
				<view class="head-search">
					...
					<input type="text" v-model="keyword" placeholder-class="input-placeholder" placeholder="搜索门店"
						@confirm="getData()" />
				</view>
			</view>

			<scroll-view scroll-y="true" class="store-list-body"
				:style="{ height: globalStoreConfig && globalStoreConfig.store_business == 'store' ? 'calc(100vh - 320rpx)' : '' }">
				<view :class="['store-item', { active: globalStoreInfo && item.store_id == globalStoreInfo.store_id }]"
					v-for="(item, index) in dataList" :key="index" @click="storeTap(item)">
					...
					success: res => {
						...
						if (res.code == 0 && res.data) {
							...
							this.dataList.forEach(item => {
								item.show_address = item.full_address.replace(/,/g, ' ') + ' ' + item
									.address;
							});
						} else {
							this.$util.showToast({
