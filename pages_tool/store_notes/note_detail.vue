	...
	<view class="goods-detail">
		<view class="goods-item" v-if="noteType == 'goods_item'">
			...
			<text class="item-title"
				v-if="goodsItemInfo.is_show_release_time == 1">{{ goodsItemInfo.note_title }}</text>
			<view class="item-lightspot" v-if="goodsItemInfo.goods_highlights.length">
				<text v-for="(item, index) in goodsItemInfo.goods_highlights" :key="index"
					class="color-base-bg">{{ item }}</text>
			</view>
			<text class="item-time"
				v-if="goodsItemInfo.is_show_release_time == 1">{{ $util.timeStampTurnTime(goodsItemInfo.create_time, 'time') }}</text>
			...
			<view class="item-action">
				<view class="action-left">
					...
					<text @click="giveLike" v-if="giveLikeIdent"
						class="iconfont icon-dianzan1 active color-base-text"></text>
					<!-- #ifdef MP -->
					<button type="primary" open-type="share" class="iconfont icon-share"></button>
					<!-- #endif -->
				</view>
				<button class="color-base-bg action-right" @click="redirectToGoods(goodsItemInfo)"
					v-if="goodsItemInfo.goods_list.length">购买</button>
			</view>
		</view>
		<!-- 掌柜说 -->
		<view class="shop-said" v-else-if="noteType == 'shop_said'">
			<text class="said-title">{{ shopSaidInfo.note_title }}</text>
			<text class="said-time"
				v-if="shopSaidInfo.is_show_release_time == 1">{{ $util.timeStampTurnTime(shopSaidInfo.create_time, 'time') }}</text>
			<view class="said-content"><rich-text :nodes="shopSaidInfo.note_content"></rich-text></view>
			<view class="said-goods">
				<view class="commodity-item" v-if="shopSaidInfo.goods_list"
					v-for="(goodsItme, goodsIndex) in shopSaidInfo.goods_list" :key="goodsIndex"
			...
			<view class="said-action">
				<text @click="giveLike" v-if="!giveLikeIdent" class="iconfont icon-dianzan"></text>
				<text @click="giveLike" v-if="giveLikeIdent"
					class="iconfont icon-dianzan1 color-base-text active"></text>
				<!-- #ifdef MP -->
				<button type="primary" open-type="share" class="iconfont icon-share"></button>
				<!-- #endif -->
			</view>
...
<script>
	...
	export default {
			...
			if (options.scene) {
				...
				if (sceneParams.length) {
					sceneParams.forEach(item => {
						...
						if (item.indexOf('m') != -1) uni.setStorageSync('source_member', item.split(
							'-')[1]);
		...
		methods: {
					...
					success: res => {
						if (res.code == 0 && res.data) {
							...
							if (this.noteType == 'goods_item') {
								...
								if (this.goodsItemInfo.note_content) this.goodsItemInfo
									.note_content =
									htmlParser(this.goodsItemInfo.note_content);
								//获取分享图片
								if (this.goodsItemInfo.goods_image) {
									this.shareImg = this.$util.img(this.goodsItemInfo
										.goods_image);
								} else {
								...
								if (this.goodsItemInfo.goods_list.length) {
									this.goodsItemInfo.goods_image = this.goodsItemInfo
										.goods_list[0]
										.goods_image.split(',')[0];
								} else {
									this.goodsItemInfo.goods_image = this.$util
										.getDefaultImage().goods;
								}

								if (this.goodsItemInfo.goods_highlights) this.goodsItemInfo
									.goods_highlights =
									this.goodsItemInfo.goods_highlights.split(',');
							} else {
								this.shopSaidInfo = res.data;
								if (this.shopSaidInfo.note_content) this.shopSaidInfo
									.note_content =
									htmlParser(this.shopSaidInfo.note_content);
							}
						} else {
				...
				this.$api.sendRequest({
					...
					success: res => {
						...
						if (res.code == 0 && res.data > 0) {
							if (this.noteType != 'goods_item')
								this.shopSaidInfo.dianzan_num = this.giveLikeIdent ? this
								.shopSaidInfo
								.dianzan_num - 1 : this.shopSaidInfo.dianzan_num + 1;
							else {
								this.goodsItemInfo.dianzan_num = this.giveLikeIdent ? this
									.goodsItemInfo
									.dianzan_num - 1 : this.goodsItemInfo.dianzan_num +
									1;
							}
			...
			imageError() {
				if (this.goodsItemInfo.goods_image) this.goodsItemInfo.goods_image = this.$util.getDefaultImage()
					.goods;
				this.$forceUpdate();
			}
		}
