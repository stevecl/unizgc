	...
	<view class="container">
		<view class="content">
			...
			<view class="get-coupon">
				...
				<view class="color-base-text coupon-price" v-else-if="info.type == 'discount'">{{ info.discount }}折
				</view>
				<text v-if="info.at_least > 0" class="color-tip">满{{ info.at_least }}元使用</text>
				<text v-else class="color-tip">无门槛优惠券</text>
				<view v-if="info.validity_type == 0" class="color-tip">有效期至 {{ $util.timeStampTurnTime(info.end_time) }}
				</view>
...
<script>
	export default {
		...
		methods: {
			 ...
			 */
			setPublicShare() {
				let shareUrl = this.$config.h5Domain + '/pages_tool/goods/coupon_receive?coupon_type_id=' + this
					.couponTypeId;
