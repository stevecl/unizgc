			...
			<view class="store-wrap">
				...
				<view class="pick-block first-pick-block">
					...
					<block v-if="orderData.buyer_ask_delivery_time && orderData.buyer_ask_delivery_time != 0">
						<input class="last-child" disabled :value="
				...
				<view class="pickup-code-info">
					...
					<view class="code">
						<image :src="$util.img(orderData.pickup_barcode)"
							@click="previewMedia($util.img(orderData.pickup_barcode))" class="barcode" mode="widthFix">
						</image>
						<image :src="$util.img(orderData.pickup)" @click="previewMedia($util.img(orderData.pickup))"
							class="qrcode" mode="widthFix"></image>
					</view>
				</view>
			</view>
		</template>
		...
		onShow() {
			...
			if (uni.getStorageSync('token')) {
				...
				this.$util.redirectTo('/pages_tool/login/login', {
					back: '/pages/order/detail?order_id=' + this.orderId + '&merchant_trade_no=' + this
						.merchantTradeNo
				});
			}
		},
				...
				this.$api.sendRequest({
					...
					success: res => {
						if (res.code >= 0) {
							...
							if (res.data.buyer_ask_delivery_time && res.data.buyer_ask_delivery_time.indexOf(
									'立即配送') != -1)
								res.data.buyer_ask_delivery_time = res.data.buyer_ask_delivery_time.replace(
									'立即配送', '');
							let num = 0;
