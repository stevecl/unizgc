			...
			<scroll-view class="scroll-view" :scroll-y="true" :show-scrollbar="false" :refresher-enabled="true"
							...
							<view class="cart-goods" @touchstart="touchS($event)" @touchend="touchE($event, item)">
								<view class="goods-wrap" :class="{ edit: item.edit }">
									...
									<view class="goods-info">
										<view>
											<view @click="toGoodsDetail(item)" class="goods-name">{{ item.goods_name }}
											</view>
											<view class="sku-wrap">
												<view class="sku">
										...
										<view class="goods-sub-section">
											...
											<uni-number-box :min="item.min_buy > 1 ? item.min_buy : 1"
												...
												size="small" :value="initNum(item)" :modifyFlag="modifyFlag"
										...
										<view class="discount-wrap" v-if="manjian && manjian['sku_' + item.sku_id]">
											...
											<scroll-view scroll-x="true" class="scroll-view">
												<block v-for="(mitem, key) in manjian['sku_' + item.sku_id]" :key="key">
													<text
														v-if="mitem.discount_money">{{ mitem.limit }}减{{ mitem.discount_money }}</text>
				...
				<block v-else>
					<view class="cart-empty">
						...
						<button type="primary" size="mini" class="button mini" v-if="!token"
